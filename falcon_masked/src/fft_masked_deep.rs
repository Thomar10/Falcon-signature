...
pub fn secure_fft<const ORDER: usize>(f: &mut [[fpr; ORDER]], logn: u32) {
    let mut u: u32 = 1;
    let mut m: usize = 2;
    ...
    while u < logn {
        ...
        while i1 < hm {
            ...
            while j < j2 {
                ...
                y_re = f[j + ht];
                y_im = f[j + ht + hn];
                (y_re, y_im) = secure_fpc_mul(&y_re, &y_im, &s_re, &s_im);
                (f[j], f[j + hn]) = secure_fpc_add(&x_re, &x_im, &y_re, &y_im);
                (f[j + ht], f[j + ht + hn]) = secure_fpc_sub(&x_re, &x_im, &y_re, &y_im);
