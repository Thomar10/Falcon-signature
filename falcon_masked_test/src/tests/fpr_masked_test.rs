...
mod tests {
    ...
    fn fpr_inv_test() {
        for _ in 0..1000000 {
            ...
            let inv_shares: [fpr; ORDER] = fpr_inv(&shares_x, &mut rng);
    ...
    fn fpr_sqrt_test() {
        for _ in 0..10000 {
            ...
            let (x, _) = create_masked(&mut shares_x, &mut shares_y);
            let mut rng = RngBoth { hal_rng: None, rust_rng: Some(thread_rng()) };
            let add_shares: [fpr; ORDER] = fpr_sqrt(&shares_x, &mut  rng);
            let (xx, _) = reconstruct(&add_shares, &shares_y);
    ...
    fn fpr_div_test() {
        for _ in 0..1000000 {
            ...
            let (x, y) = create_masked(&mut shares_x, &mut shares_y);
            let mut rng = RngBoth { hal_rng: None, rust_rng: Some(thread_rng()) };
            let div_shares: [fpr; ORDER] = fpr_div(&shares_x, &shares_y, &mut rng);
