...
fn fpc_add_test() {
    for _ in 0..1000 {
        ...
        let (result_re, result_im) =
            secure_fpc_add::<2>(&x_re_mask, &x_im_mask, &y_re_mask, &y_im_mask, &mut rng);
...
fn fpc_mul_test() {
    for _ in 0..1000 {
        ...
        let (result_re, result_im) =
            secure_fpc_mul::<2>(&x_re_mask, &x_im_mask, &y_re_mask, &y_im_mask, &mut rng);
        check_eq_fpr(expected_re, result_re[0] ^ result_re[1]);
        check_eq_fpr(expected_im, result_im[0] ^ result_im[1]);
    }
}
...
fn fpc_sub_test() {
    for _ in 0..1000 {
        ...
        let (result_re, result_im) =
            secure_fpc_sub::<2>(&x_re_mask, &x_im_mask, &y_re_mask, &y_im_mask, &mut rng);
        check_eq_fpr(expected_re, result_re[0] ^ result_re[1]);
        check_eq_fpr(expected_im, result_im[0] ^ result_im[1]);
    }
}
...
fn test_fft() {
    ...
    for _ in 0..5 {
        ...
        let mut rng = RngBoth { hal_rng: None, rust_rng: Some(thread_rng()) };
        secure_fft(&mut f_shares, LOGN, &mut rng);
        fft(&mut f, LOGN);
