...
void    AES256_ECB2(unsigned char *key, unsigned char *ctr, unsigned char *buffer);
...
seedexpander2(AES_XOF_struct *ctx, unsigned char *x, unsigned long xlen)
{
    ...
    while ( xlen > 0 ) {
        ...
        AES256_ECB2(ctx->key, ctx->ctr, ctx->buffer);
        ctx->buffer_pos = 0;
...
void
AES256_ECB2(unsigned char *key, unsigned char *ctr, unsigned char *buffer)
{
    EVP_CIPHER_CTX *ctx;
    ...
    if(!(ctx = EVP_CIPHER_CTX_new())) handleErrors2();
...
void randombytes_init2(unsigned char *entropy_input,
  ...
  AES256_CTR_DRBG_Update2(seed_material, DRBG_ctx2.Key, DRBG_ctx2.V);
  DRBG_ctx2.reseed_counter = 1;
}


int randombytes2(unsigned char *x, unsigned long long xlen) {
  ...
  while (xlen > 0) {
    ...
    AES256_ECB2(DRBG_ctx2.Key, DRBG_ctx2.V, block);
    if (xlen > 15) {
      memcpy(x + i, block, 16);
      i += 16;
      xlen -= 16;
  ...
  AES256_CTR_DRBG_Update2(NULL, DRBG_ctx2.Key, DRBG_ctx2.V);
