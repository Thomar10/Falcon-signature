...
pub fn solve_ntru_intermediate_depth1(profile: &NtruProfile, logn_top: usize, f: &[i8], g: &[i8], tmp: &mut [u32]) -> bool {
    ...
    let (mut rt4, rt1) = inter_fxr.split_at_mut(n);
    ...
    loop {
        ...
        (Gt, inter) = inter.split_at_mut(llen * n);
        let nrt3 = bytemuck::cast_slice_mut::<u32, fxr>(inter);
        let (_, inter_fxr) = nrt3.split_at_mut(2 * n);
        let k = bytemuck::cast_slice_mut::<fxr, i32>(inter_fxr);
        let (k, t2) = k.split_at_mut(n);
        let scale_k = scale_FG - scale_fg;
...
pub fn solve_ntru_depth0(profile: &NtruProfile, logn: usize, f: &[i8], g: &[i8], tmp: &mut [u32]) -> bool {
    ...
    let rt3 = bytemuck::cast_slice_mut::<u32, fxr>(t3);
    for u in 0..n {
        rt3[u] = (t2[u] as i32 as u64) << 22;
    }
    vect_fft(logn, rt3);
    let inter = bytemuck::cast_slice_mut::<u32, fxr>(inter2);
    let (rt2, rt3) = inter.split_at_mut(hn);
    rt2.copy_from_slice(&rt3[0..hn]);
    for u in 0..n {
        rt3[u] = (t1[u] as i32 as u64) << 22;
    }
